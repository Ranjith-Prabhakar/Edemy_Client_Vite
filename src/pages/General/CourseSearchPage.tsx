...
const CourseSearchPage = () => {
  const { key, sort, filter } = useLocation().state.queryData;
  ...
  useEffect(() => {
    if (isSuccess) {
      setData((prevData) => [
        ...prevData,
        ...((searchData?.data as ICourse[]) || []),
      ]);
      setHasMore(searchData?.hasMore as boolean);
      setLoading(false);
    }
  }, [isSuccess]);
      ...
      <div className="min-h-screen mt-10">
          ...
          {!data.length && !loading && !hasMore ? (
            <div className="w-screen h-screen  flex justify-center items-start mt-44">
              <h1 className="text-4xl font-extrabold text-center">
                No Courses Have Have Found For This Query..
              </h1>
            </div>
          ) : (
            data.map((item, index) => (
        ...
        {loading && hasMore && (
          <div role="status" className="flex justify-center mt-10 pb-10">
            <svg
              aria-hidden="true"
              className="inline w-8 h-8  animate-spin text-gray-600  fill-gray-300"
              viewBox="0 0 100 101"
              fill="none"
